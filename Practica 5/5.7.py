# Escribir un programa que juegue con el usuario a adivinar un número. El programa
# debe generar un número al azar entre 1 y 500 y el usuario debe adivinarlo. Para
# eso, cada vez que se introduce un valor se muestra un mensaje indicando si el número
# que tiene que adivinar es mayor o menor que el ingresado. Cuando consiga
# adivinarlo, se debe imprimir en pantalla la cantidad de intentos que le tomó hallar
# el número. Si el usuario introduce algo que no sea un número se mostrará un
# mensaje en pantalla y se lo contará como un intento más.

import random 
#FUNCIONES:
def adivinar(nro):
    try:
        intentos = 0
        num = int(input("Ingrese un numero entre 1 y 500: "))
        while num != nro:
            num = int(input("Ingrese un numero entre 1 y 500: "))
            try:
                if num > nro:
                    intentos += 1
                    print("El numero ingresado es mayor que el numero que busca")
                elif num < nro:
                    intentos += 1
                    print("El numero ingresado es menor que el numero que busca")
                else:
                    print("Felicitaciones, encontró el numero")
            except ValueError:
                print("Error, se esperaba un número válido")
                intentos += 1
    except ValueError:
        print("Error, ingreso un número inválido")
    return intentos

#PROGRAMA PRINCIPAL:
def main():
    print("Se generó un numero, intente adivinarlo")
    nro = random.randint(1,500)
    print(f"La cantidad de intentos que tuvo fue de {adivinar(nro)}")

if __name__ == "__main__":
    main()